...
function App() {
  ...
  const updateStreaks = (roundScores) => {
    ...
    players.forEach(player => {
      ...
      const isWinner = player === winner && roundScores[player] > 0;
      const isLoser = player === loser && roundScores[player] < 0;
      ...
      if (isWinner) {
        ...
        // Reset streak nếu không thắng cũng không thua chót
      ...
      newStreaks[player].history.push({
        round: logs.length + 1,
        type: isWinner ? 'win' : isLoser ? 'lose' : 'normal',
